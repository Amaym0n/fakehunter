...
async def create_job(job: JobCreate, db: Session = Depends(dependency=get_db)) -> Job:
    return await create_new_job(job=job, db=db)


@jobs_router.get(path='/job/{job_id}', status_code=HTTPStatus.OK)
async def retrieve_job(job_id: conint(gt=0), db: Session = Depends(dependency=get_db)) -> Job:
